...
def get_Q_exact(decoherence_time_mu_s: int) -> np.matrix:
    # Target value inside the N-th root based on the equation above
    # equivalent to (4/(3e) - 1/3)
    target = (4.0 / np.e - 1.0) / 3.0

    # N-th root calculation
    root_val = target ** (1.0 / decoherence_time_mu_s)
...
def extract_lines_by_keyword(fn: str, kw: str) -> list[str]:
    ...
    if not os.path.exists(fn):
        # return empty if file not found to prevent crashing
        return []
    with open(fn, "r") as f:
        for line in f:
...
number_of_requested_bellpairs = 100_000 # short
