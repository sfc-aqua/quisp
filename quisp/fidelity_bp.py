...
for line in lines:
    ...
    for link in ["MIM","MSM"]:
        ...
        if line[0] == f"{link}_depolarizing_purified":
            results[link][1] = [float(x) for x in line[1:]]
        if line[0] == f"{link}_depolarizing_purified_two":
            results[link][2] = [float(x) for x in line[1:]]
        if line[0] == f"{link}_depolarizing_purified_three":
            results[link][3] = [float(x) for x in line[1:]]
        if line[0] == f"{link}_depolarizing_purified_four":
