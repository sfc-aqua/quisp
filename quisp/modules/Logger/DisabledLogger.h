...
class DisabledLogger : public ILogger {
 public:
  ...
  void logBellPairInfo(const std::string& event_type, QNodeAddr partner_addr, quisp::modules::QNIC_type qnic_type, int qnic_index, int qubit_index) override { return; }
  void setModule(omnetpp::cModule const* const mod) override { return; }
  void setQNodeAddress(QNodeAddr addr) override { return; }
};
}  // namespace quisp::modules::Logger
