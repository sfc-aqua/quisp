...
class Strategy : public quisp_test::TestComponentProviderStrategy {
 public:
  Strategy(cModule *_qnode) : parent_qnode(_qnode) {}
  cModule *getQNode() override { return parent_qnode; }

 private:
  cModule *parent_qnode;
};
...
class MockTopoLink : public cTopology::Link {
 public:
...
class MockTopologyNode : public cTopology::Node {
 public:
  ...
  MockTopologyNode(cTopology::LinkOut *_outPath) { outPath = _outPath; }
};

class MockTopology : public cTopology {
 public:
  MockTopology(MockTopologyNode *node) : cTopology(), node(node) {}
  MockTopologyNode *node;
  cTopology::Node *getNodeFor(cModule *mod) const override { return node; };
};

class MockQRSA : public omnetpp::cModule {
 public:
  MockQRSA(MockQNode *parent) : parent(parent) {}
  MockQNode *parent;

  cModule *getParentModule() const override { return dynamic_cast<omnetpp::cModule *>(parent); }
};

class RoutingDaemonTestTarget : public quisp::modules::RoutingDaemon {
 public:
  // using quisp::modules::RoutingDaemon::getParentModule;
  using quisp::modules::RoutingDaemon::generateRoutingTable;
  ...
  RoutingDaemonTestTarget(cModule *parent_qnode, int address) : RoutingDaemon() {
...
TEST(RoutingDaemonTest, DISABLED_InitTestNoTopology) {
...
TEST(RoutingDaemonTest, DISABLED_InitTopologyTest) {
  auto sim = prepareSimulation();
  auto *mock_qnode1 = new MockQNode{123, 100, false};
  auto *mock_qnode2 = new MockQNode{456, 100, false};
  auto *mock_qrsa = new MockQRSA(mock_qnode1);
  RoutingDaemonTestTarget c1{mock_qnode1, 123};
  sim->registerComponent(&c1);
...
TEST(RoutingDaemonTest, generateRoutingTable) {
  auto sim = prepareSimulation();
  auto *mock_qnode1 = new MockQNode{123, 100, false};
  auto *mock_qnode2 = new MockQNode{456, 100, false};
  auto *mock_qrsa = new MockQRSA(mock_qnode1);

  auto rd = new RoutingDaemonTestTarget{mock_qrsa, 456};
  ...
  MockGate *mgate = new MockGate;
  MockChannel *mchannel = new MockChannel;
  auto *outPath = new MockTopoLink();
  auto *node = new MockTopologyNode((cTopology::LinkOut *)outPath);
  node->moduleId = mock_qnode1->getId();
  outPath->srcNode = node;
  outPath->srcGateId = 0;
  outPath->destGateId = 1;

  cTopology *topo = new MockTopology(node);
  topo->extractByParameter("included_in_topology", "\"yes\"");
