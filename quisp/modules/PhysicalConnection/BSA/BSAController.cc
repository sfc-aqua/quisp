...
void BSAController::initialize(int stage) {
  ...
  if (passive) {
    ...
    error("Set receiver parameter of BSA to true or false.");
  }
}

// Initialization of the BSA module inside a QNode.
// The initialization will be a little bit different from the stand-alone BSA module.
// For example, you don't need to check both of the neighbors because it is inside a QNode.
void BSAController::internodeInitializer() {
  checkNeighborAddress(true);
  checkNeighborBuffer(true);
  updateIDE_Parameter(true);

  accepted_burst_interval = (double)1 / (double)photon_detection_per_sec;
  auto *generatePacket = new BSANotificationTimer("BsaStart");
  scheduleAt(simTime() + par("initial_notification_timing_buffer"), generatePacket);
}

// Initialization of the stand-alone BSA module.
void BSAController::standaloneInitializer() {
  // Just in case, check if the 2 quantum port of the node
  if (getParentModule()->gateSize("quantum_port") != 2) {
    error("No more or less than 2 neighbors are allowed for BSA.");
    endSimulation();
  }
...
void BSAController::sendNotifiers() {
  ...
  try {
    ...
    error("Error in BSA_Controller.cc. It does not have port named toRouter_port.");
    endSimulation();
...
void BSAController::handleMessage(cMessage *msg) {
  if (dynamic_cast<BSANotificationTimer *>(msg)) {
    sendNotifiers();
    auto *notification_timer = new BSANotificationTimer("BsaStart");
...
void BSAController::checkNeighborAddress(bool receiver) {
  if (receiver) {
    try {
      ...
      error("Error in BSA_Controller.cc when getting neighbor addresses. Check internodeInitializer.");
      endSimulation();
    }
  } else {  // Controller in a Stand alone BSA node
    try {
      ...
      error("Error in BSA_Controller.cc. Your stand-alone BSA module may not connected to the neighboring node (quantum_port).");
      endSimulation();
    }
  }
}
...
void BSAController::checkNeighborBuffer(bool receiver) {
  if (receiver) {
    try {
      ...
      error("Error in BSA_Controller.cc. BSA couldnt find parameter num_buffer in the neighbor's qnic.");
      endSimulation();
    }
  } else {
    try {
...
void BSAController::updateIDE_Parameter(bool receiver) {
  try {
    photon_detection_per_sec = (int)par("photon_detection_per_sec");
    if (photon_detection_per_sec <= 0) {
      error("Photon detection per sec for BSA must be more than 0.");
    }
...
BSMtimingNotifier *BSAController::generateNotifier(double time, double speed_of_light_in_channel, double distance_to_neighbor, int destAddr, double accepted_burst_interval,
...
CombinedBSAresults *BSAController::generateNotifier_c(double time, double speed_of_light_in_channel, double distance_to_neighbor, int destAddr, double accepted_burst_interval,
                                                      int photon_detection_per_sec, int max_buffer) {
...
// The farther node emits it instantaneously, while the closer one needs to wait because 2 photons need to arrive at BSA simultaneously.
double BSAController::calculateEmissionStartTime(double time, double distance_to_node, double c) {
  // distance_to_node is the distance to BSA to self
...
double BSAController::calculateTimeToTravel(double distance, double c) { return (distance / c); }

cModule *BSAController::getQNode() {
...
void BSAController::pushToBSAresults(bool attempt_success) {
  int prev = getStoredBSAresultsSize();
...
int BSAController::getStoredBSAresultsSize() { return results.size(); }
void BSAController::clearBSAresults() { results.clear(); }
...
void BSAController::sendBSAresultsToNeighbors() {
  if (!passive) {
...
// When CM receives it, it will also have to update the max_buffer of BSAController, to know when the emission end and send the classical BSAresults to the neighboring EPPS.
void BSAController::setMax_buffer(int buffer) {
