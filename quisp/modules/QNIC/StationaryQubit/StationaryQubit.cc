...
void StationaryQubit::setFree(bool consumed) {
  ...
  EV << "Freeing this qubit!!!" << this <<" at qnode: "<<node_address<<" qnic_type: "<<qnic_type<<" qnic_index: "<<qnic_index<<"\n";
  // GUI part
  if (hasGUI()) {
    if (consumed) {
      bubble("Consumed!");
...
void StationaryQubit::setEntangledPartnerInfo(StationaryQubit *partner) {
  ...
  if (hasGUI()) {
    char text[32];
    sprintf(text, "I'm entangled with %d", entangled_partner->node_address);
    bubble(text);
  }
