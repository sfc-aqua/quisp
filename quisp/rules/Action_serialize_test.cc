...
TEST(ActionTest, EntanglementSwapping_serialize) {
  prepareSimulation();
  // num measure
  ...
  json swapping_json = swapping->serialize();
...
TEST(ActionTest, Wait_serialize) {
  prepareSimulation();
  // num measure
  auto wait = std::make_unique<Wait>(1, QNIC_R, 13);
  json wait_json = wait->serialize();
...
TEST(ActionTest, Tomography_serialize) {
  prepareSimulation();
  // num measure
  auto tomography = std::make_unique<Tomography>(1000, 3, QNIC_E, 15);
  json tomography_json = tomography->serialize();
