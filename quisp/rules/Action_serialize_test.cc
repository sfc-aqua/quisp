...
TEST(ActionTest, Purification_serialize_json) {
  ...
  auto purification = std::make_unique<Purification>(PurType::DSDA, QNodeAddr{3});
  ...
  EXPECT_EQ(purification_json["options"]["interface"][0]["partner_address"], "0.3");
}
...
TEST(ActionTest, Purification_deserialize_json) {
  ...
  EXPECT_EQ(qnic_interface.partner_addr, QNodeAddr{1});
}
...
TEST(ActionTest, EntanglementSwapping_serialize_json) {
  ...
  EXPECT_EQ(swapping_json["options"]["interface"][0]["partner_address"], "0.1");
  EXPECT_EQ(swapping_json["options"]["interface"][1]["partner_address"], "0.3");
}
...
TEST(ActionTest, EntanglementSwapping_deserialize_json) {
  ...
  EXPECT_EQ(qnic_interface_left.partner_addr, QNodeAddr{1});
  auto qnic_interface_right = empty_swapping->qnic_interfaces.at(1);
  EXPECT_EQ(qnic_interface_right.partner_addr, QNodeAddr{3});
}

TEST(ActionTest, Wait_serialize_json) {
  prepareSimulation();
  // num measure
  auto wait = std::make_unique<Wait>(QNodeAddr{13});
  json wait_json = wait->serialize_json();
  EXPECT_EQ(wait_json["type"], "wait");
  EXPECT_EQ(wait_json["options"]["interface"][0]["partner_address"], "0.13");
}
...
TEST(ActionTest, Tomography_serialize_json) {
  prepareSimulation();
  // num measure
  auto tomography = std::make_unique<Tomography>(1000, QNodeAddr{2}, QNodeAddr{3});
  ...
  EXPECT_EQ(tomography_json["options"]["owner_address"], "0.2");
}
...
TEST(ActionTest, Tomography_deserialize_json) {
  ...
  EXPECT_EQ(empty_tomography->owner_address, QNodeAddr{2});
  EXPECT_EQ(qnic_interface.partner_addr, QNodeAddr{1});
}
}  // namespace
