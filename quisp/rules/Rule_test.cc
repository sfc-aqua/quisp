...
TEST(RuleTest, serialize_json_purification_rule) {
  ...
  auto enough_resource_clause = std::make_unique<EnoughResourceConditionClause>(1, QNodeAddr{1});
  condition->addClause(std::move(enough_resource_clause));
  // purification_type, partner_addr, qnic_type, qnic_id
  auto action = std::make_unique<Purification>(PurType::DSSA, QNodeAddr{1}, 2);
  ...
  EXPECT_EQ(purification_json["interface"][0]["partner_address"], "0.1");
  ...
  EXPECT_EQ(clause_json["options"]["interface"]["partner_address"], "0.1");
  ...
  EXPECT_EQ(action_json["options"]["interface"][0]["partner_address"], "0.1");
}
...
TEST(RuleTest, serialize_json_swapping_rule) {
  ...
  EXPECT_EQ(swapping_json["interface"][0]["partner_address"], "0.1");
  EXPECT_EQ(swapping_json["interface"][1]["partner_address"], "0.3");
  ...
  EXPECT_EQ(clause1_json["options"]["interface"]["partner_address"], "0.1");
  ...
  EXPECT_EQ(clause2_json["options"]["interface"]["partner_address"], "0.3");

  // action: entanglement swapping ("swapping")
  auto action_json = swapping_json["action"];
  EXPECT_EQ(action_json["type"], "swapping");
  EXPECT_EQ(action_json["options"]["interface"][0]["partner_address"], "0.1");
  EXPECT_EQ(action_json["options"]["interface"][1]["partner_address"], "0.3");
}
...
TEST(RuleTest, deserialize_json_purification_rule) {
  ...
  auto purification = std::make_unique<Rule>(QNodeAddr{1}, -1, -1);
  purification->setName("purification");
  auto condition = std::make_unique<Condition>();
  auto enough_resource_clause = std::make_unique<EnoughResourceConditionClause>(1, QNodeAddr{1});
  condition->addClause(std::move(enough_resource_clause));
  // purification_type, partner_addr shared_rule_tag
  auto action = std::make_unique<Purification>(PurType::DSSA, QNodeAddr{1}, 3);
  // add condition and action
  purification->setCondition(std::move(condition));
  purification->setAction(std::move(action));
  std::vector<QNodeAddr> partners = {QNodeAddr{1}, QNodeAddr{3}};
  ...
  EXPECT_EQ(empty_rule->qnic_interfaces.at(0).partner_addr, QNodeAddr{1});
}
