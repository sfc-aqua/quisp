...
cPacket *SimultaneousSwappingAction::run(cModule *re) {
  ...
  if (left_measure == MeasureZResult::NO_Z_ERROR && right_measure == MeasureZResult::NO_Z_ERROR) {  // 0 0
    EV << "operation type 0, operation left I, operation right I\n";
    operation_type_left = 0;
    operation_type_right = 0;
  } else if (left_measure == MeasureZResult::NO_Z_ERROR && right_measure == MeasureZResult::HAS_Z_ERROR) {  // 0 1
    EV << "operation type 1, operation left I, operation right X\n";
    operation_type_left = 0;
    operation_type_right = 1;
  } else if (left_measure == MeasureZResult::HAS_Z_ERROR && right_measure == MeasureZResult::NO_Z_ERROR) {  // 1 0
